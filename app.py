    ...
    for event in events:
        ...
        if response == False:
            if event.message.text.lower() == 'fsm':
                send_image_message(event.reply_token, 'https://i.postimg.cc/4ykXRfCp/fsm.png')
            elif machines[event.source.user_id].state == 'user':
                send_text_message(event.reply_token, '準備好跟郭台銘一樣有錢了嗎?\nEnter "menu" to start!\nEnter "fsm" to see picture of fsm')
            elif machines[event.source.user_id].state == 'input_location':
                send_text_message(event.reply_token, 'Please enter you location')
            elif machines[event.source.user_id].state == 'input_radius':
